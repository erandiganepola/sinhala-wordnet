			...
			<table>
				<tr>
					...
					<td align="left"><input type="button" class="button"
						value="Home"
						onclick="window.location.href='ShowSynsets?action=ShowRoot'"
						align="left" /></td>
				</tr>
			</table>
		</div>
				...
				<tbody>
					...
					<c:forEach var="synset" items="${synsetList}" varStatus="loop">
						<tr class='clickableRow'
							href='EditSynsets${type}?action=ShowEditSynset&type=<c:out value="${type}"/>&id=<c:out value="${synset.getEWNId()}"/>&mongoid=<c:out value="${synset.getId()}"/>'>
