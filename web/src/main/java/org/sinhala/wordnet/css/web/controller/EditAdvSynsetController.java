...
@Controller
@RequestMapping("/EditSynsetsadv")
public class EditAdvSynsetController {
	
	
	@RequestMapping(method = RequestMethod.GET, params = { "action=ShowEditSynset", "type=adv" })
	public String showAdvEditSynset(@RequestParam(value = "id", required = false) String id, ModelMap model, @RequestParam(value = "type", required = false) String type,@RequestParam(value = "mongoid", required = false) String mongoid) {
		...
		if (id != null && !"".equals(id)) {
			...
			mongoCastSynset = (AdverbSynset) mongoSynsetConvertor.OverWriteByMongo(castSynset,mongoid);
	...
	public String editSynset(@ModelAttribute AdverbSynset synset, ModelMap model) {
		...
		AdverbSynset aSynset = (AdverbSynset)synset;
		...
		return showAdvEditSynset(String.valueOf(synset.getOffset()), model, "adv","");
	}
